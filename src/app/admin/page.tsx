...
export default async function AdminDashboard() {
  if (DEMO) {
    ...
    const stats = [
      { label: 'Total Users', value: DEMO_USERS.length, icon: '👥' },
      { label: 'Total Posts', value: DEMO_POSTS.length, icon: '🖥️' },
      { label: 'Total Ratings', value: DEMO_POSTS.reduce((a, p) => a + p.ratingCount, 0), icon: '⭐' },
      { label: 'Comments', value: DEMO_POSTS.reduce((a, p) => a + p.commentCount, 0), icon: '💬' },
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="bg-card border border-border rounded-xl p-6 text-center text-muted-foreground">
          <p className="text-sm">Full admin functionality is available after deployment with a real database.</p>
        </div>
      </div>
    )
  }
  ...
  const [totalUsers, totalPosts, totalRatings, totalComments, pendingReports, recentPosts, recentUsers, recentLogs] = await Promise.all([
    ...
    prisma.user.findMany({ orderBy: { createdAt: 'desc' }, take: 5, select: { id: true, username: true, email: true, createdAt: true, role: true } }),
    prisma.adminLog.findMany({ orderBy: { createdAt: 'desc' }, take: 10, include: { admin: { select: { username: true } } } }),
  ])
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        {stats.map(s => (
          <div key={s.label} className={`bg-card border rounded-xl p-4 ${(s as any).alert ? 'border-destructive/50' : 'border-border'}`}>
            <div className="text-2xl mb-1">{s.icon}</div>
            <div className={`text-2xl font-black ${(s as any).alert ? 'text-destructive' : ''}`}>{s.value.toLocaleString()}</div>
            <div className="text-xs text-muted-foreground mt-0.5">{s.label}</div>
          </div>
        ))}
      </div>
              ...
              return (
                <div key={post.id} className="bg-card border border-border rounded-lg p-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={`/post/${post.id}`} className="text-sm font-medium truncate block hover:text-primary transition-colors">{post.title}</Link>
          ...
          <div className="space-y-2">
            {recentUsers.map(user => (
              <div key={user.id} className="bg-card border border-border rounded-lg p-3 flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center text-xs font-bold">{user.username[0].toUpperCase()}</div>
                <div className="flex-1 min-w-0">
                  <Link href={`/u/${user.username}`} className="text-sm font-medium hover:text-primary transition-colors">@{user.username}</Link>
                  <p className="text-xs text-muted-foreground truncate">{user.email}</p>
                </div>
                <span className={`text-xs px-1.5 py-0.5 rounded ${user.role === 'ADMIN' ? 'bg-primary/20 text-primary' : user.role === 'BANNED' ? 'bg-destructive/20 text-destructive' : 'bg-secondary text-muted-foreground'}`}>{user.role}</span>
              </div>
            ))}
          </div>
        </div>
