...
export default async function PostPage({ params }: Props) {
  ...
  const userRating = (post.ratings as any)?.[0]?.score ?? null
  const { grade, color } = gradeFromScore(post.avgRating)
  const specs = ['cpu', 'gpu', 'ram', 'storage', 'peripherals', 'deskChair']
    .filter(k => (post as any)[k])
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div>
            ...
            <div className="flex items-center gap-3 text-sm text-muted-foreground">
              ...
              {post.buildCost && (
                <>
                  <span>·</span>
                  <span>💰 ~${post.buildCost.toLocaleString()}</span>
                </>
              )}
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {specs.map(key => (
                  <div key={key} className="bg-secondary/50 rounded-lg p-3">
                    ...
                    <p className="text-sm font-medium">{(post as any)[key]}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <CommentThread postId={post.id} comments={comments as any} />
        </div>
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            <Link href={`/u/${post.author.username}`} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
              ...
              <div>
                ...
                {post.author.bio && (
                  <p className="text-xs text-muted-foreground mt-0.5 line-clamp-2">{post.author.bio}</p>
                )}
              </div>
            </Link>
          </div>
